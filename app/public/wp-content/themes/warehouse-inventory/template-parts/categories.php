...
<div id="categoryModal" class="modal-overlay" style="display:none;">
    <div class="modal" style="position: absolute !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; border-radius: 8px !important; width: 90% !important; max-width: 500px !important; max-height: 80vh !important; overflow-y: auto !important; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3) !important;">
                ...
                <table class="modal-form">
                    ...
                    <tr>
                        <td style="padding: 8px 0 !important; vertical-align: top !important;">
                            ...
                            <textarea id="categoryDescription" name="description" rows="3" placeholder="Optional description for this category..." class="form-input"></textarea>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0 !important; vertical-align: top !important;">
                            <table style="width: 100% !important; border-collapse: collapse !important;">
                                <tr>
                                    ...
                                    <td style="vertical-align: top !important;">
                                        ...
                                        <select id="categoryIcon" name="icon" class="form-select">
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" onclick="submitCategory(event)" id="submitBtn">Add Category</button>
        </div>
    </div>
</div>

<!-- Category Items Modal -->
<div id="categoryItemsModal" class="modal-overlay" style="display:none;">
    <div class="modal">
        <div class="modal-header">
            <h3 id="categoryItemsTitle" class="modal-title">Items in Category</h3>
            <button class="modal-close" onclick="closeCategoryItemsModal()">&times;</button>
        </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" onclick="closeCategoryItemsModal()">Close</button>
            <button type="button" class="btn btn-primary" onclick="goToInventoryFiltered()" id="goToInventoryBtn">Go to Inventory</button>
        </div>
    </div>
</div>
...
// Refresh category counts after item operations
function refreshCategoryCounts() {
    console.log('Refreshing category counts...');
    
    // Reload the entire categories page to get fresh counts
...
// Accessibility: close with Escape, avoid outside-click conflicts with native selects
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        try { closeCategoryItemsModal(); } catch(_) {}
        try { closeCategoryModal(); } catch(_) {}
    }
});

// Stabilize native selects inside the category modal by converting to an inline listbox while focused
;(function(){
    function stabilize(select){
        if (!select) return;
        let opened = false;
        const maxSize = Math.min(8, Math.max(3, select.options.length));
        // Open as listbox on focus/mousedown
        ...
        select.addEventListener('mousedown', function(e){ open(); });
    ...
    document.addEventListener('DOMContentLoaded', function(){
        stabilize(document.getElementById('categoryParent'));
