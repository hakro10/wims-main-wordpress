...
define('WH_INVENTORY_VERSION', '1.1.4');
...
class WarehouseInventoryManager {
    ...
    public function admin_scripts($hook) {
        if (strpos($hook, 'warehouse') !== false) {
            ...
            // Production assets
            wp_enqueue_style('warehouse-production', get_template_directory_uri() . '/assets/css/production.css', array(), WH_INVENTORY_VERSION);
            wp_enqueue_script('warehouse-production', get_template_directory_uri() . '/assets/js/production.js', array(), WH_INVENTORY_VERSION, true);
            
            // Enhanced theme assets
            wp_enqueue_style('warehouse-animations', get_template_directory_uri() . '/assets/css/animations.css', array(), WH_INVENTORY_VERSION);
            wp_enqueue_style('warehouse-shadcn', get_template_directory_uri() . '/assets/css/shadcn-components.css', array(), WH_INVENTORY_VERSION);
            
            // Enhanced theme scripts
            wp_enqueue_script('warehouse-theme-toggle', get_template_directory_uri() . '/assets/js/theme-toggle.js', array(), WH_INVENTORY_VERSION, true);
            wp_enqueue_script('warehouse-loading', get_template_directory_uri() . '/assets/js/loading-manager.js', array(), WH_INVENTORY_VERSION, true);
            
            wp_localize_script('warehouse-admin', 'warehouse_ajax', array(
                // Relative admin-ajax URL prevents scheme mismatches in local dev
    ...
    public function frontend_scripts() {
        if (!is_user_logged_in()) {
            return; // Protect app assets from guests
        }
    ...
    public function handle_update_inventory_item() {
        ...
        error_log("=== CASCADING DELETE STARTED FOR ITEM ID: $id ===");
        ...
        try {
            ...
            error_log("Item found: " . $item->name . " (ID: $id)");

            // Count related sales records before deletion
            $sales_count = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}wh_sales WHERE item_id = %d OR inventory_item_id = %d",
                $id, $id
            ));

            error_log("Found $sales_count related sales records to delete");

            // Delete related sales records
            $sales_deleted_1 = $wpdb->delete(
                $wpdb->prefix . 'wh_sales',
                array('item_id' => $id),
                array('%d')
            );

            // Also delete by inventory_item_id if it exists
            $sales_deleted_2 = $wpdb->delete(
                $wpdb->prefix . 'wh_sales',
                array('inventory_item_id' => $id),
                array('%d')
            );

            $total_sales_deleted = $sales_deleted_1 + $sales_deleted_2;
            error_log("Deleted $total_sales_deleted sales records");

            // Delete the main inventory item
            $item_deleted = $wpdb->delete(
                $wpdb->prefix . 'wh_inventory_items',
                array('id' => $id),
                array('%d')
            );
            ...
            error_log("Successfully deleted item: " . $item->name);

            // Commit transaction
            $wpdb->query('COMMIT');

            error_log("=== CASCADING DELETE COMPLETED SUCCESSFULLY ===");

            wp_send_json_success(array(
                'message' => 'Item and all related records deleted successfully',
                'item_name' => $item->name,
                'sales_deleted' => $total_sales_deleted
            ));
    ...
    // Cleanup handlers for inactive items
    public function handle_get_inactive_items() {
        check_ajax_referer('warehouse_nonce', 'nonce');
        ...
        $inactive_items = $wpdb->get_results(
        ...
        wp_send_json_success($inactive_items);
    }
    
    public function handle_permanently_delete_item() {
        check_ajax_referer('warehouse_nonce', 'nonce');
        ...
        error_log("=== PERMANENT DELETE STARTED FOR ITEM ID: $id ===");
        
        // Start transaction
        $wpdb->query('START TRANSACTION');
        
        try {
            // Verify item exists and is inactive
            $item = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}wh_inventory_items WHERE id = %d AND status = 'inactive'",
                $id
            ));
            
            if (!$item) {
                throw new Exception('Item not found or not inactive');
            }
            
            // Delete related sales records
            $sales_deleted = $wpdb->delete(
                $wpdb->prefix . 'wh_sales',
                array('item_id' => $id),
                array('%d')
            );
            
            // Also delete by inventory_item_id if column exists
            $inventory_sales_deleted = $wpdb->delete(
                $wpdb->prefix . 'wh_sales',
                array('inventory_item_id' => $id),
                array('%d')
            );
            ...
            $total_sales_deleted = $sales_deleted + $inventory_sales_deleted;
            
            error_log("=== PERMANENT DELETE COMPLETED ===");
            error_log("Item deleted: YES");
            error_log("Sales records deleted: $total_sales_deleted");
            ...
            wp_send_json_error('Failed to permanently delete item: ' . $e->getMessage());
        }
    }
    
    public function handle_bulk_cleanup_inactive_items() {
        check_ajax_referer('warehouse_nonce', 'nonce');
        ...
        error_log("=== BULK CLEANUP STARTED ===");
        ...
        try {
            ...
            $inactive_items = $wpdb->get_results(
                "SELECT id, name FROM {$wpdb->prefix}wh_inventory_items WHERE status = 'inactive'"
            );
            
            if (empty($inactive_items)) {
                wp_send_json_success(array(
                    'message' => 'No inactive items to cleanup',
                    'items_deleted' => 0,
                    'sales_deleted' => 0
                ));
                return;
            ...
            $item_ids = array_column($inactive_items, 'id');
            $item_names = array_column($inactive_items, 'name');
            ...
            foreach ($item_ids as $item_id) {
                ...
                $sales_deleted += $wpdb->delete(
                    $wpdb->prefix . 'wh_sales',
                    array('inventory_item_id' => $item_id),
                    array('%d')
                );
            }
            ...
            $items_deleted = $wpdb->query(
                "DELETE FROM {$wpdb->prefix}wh_inventory_items WHERE status = 'inactive'"
            );
            
            if ($items_deleted === false) {
                throw new Exception('Failed to delete inactive items');
            }
            
            // Commit transaction
            $wpdb->query('COMMIT');
            
            error_log("=== BULK CLEANUP COMPLETED ===");
            error_log("Items deleted: $items_deleted");
            error_log("Sales records deleted: $sales_deleted");
            
            wp_send_json_success(array(
                'message' => "Permanently deleted $items_deleted inactive items",
            ...
            // Rollback transaction
            $wpdb->query('ROLLBACK');
            error_log("=== BULK CLEANUP FAILED: " . $e->getMessage() . " ===");
            wp_send_json_error('Failed to cleanup inactive items: ' . $e->getMessage());
        }
    }
}
